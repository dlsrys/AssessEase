<?php 
  ...
  if(isset($_POST['done'])){
    ...
    //sql statements for updating and inserting data
		$update = "UPDATE quiz SET title='$title', descri='$dsc' WHERE quizID ='$qid'";
    $insertData = "UPDATE quiz set StartDate='$open', EndDate='$close',saved=1 WHERE quizID ='$qid'";

      //when done, execute the sql        
      if ($result = mysqli_query($con,$update)){
        if($resultInsert = mysqli_query($con,$insertData)){
          ...
          if (mysqli_num_rows($ques)==0){       
            ...
            echo "<script>alert('Quiz created. Edit anytime!'); window.location.href = 'quizzes.php';</script>";
    ...
    if(isset($_POST['return'])){
        //if random is null, the user never saved. delete the quiz
        if(is_null($quiz['saved'])){
            //delete the quiz if never saved
            $delete = "DELETE FROM quiz WHERE quizID='$qid'";
            $resDel = mysqli_query($con,$delete);
            header('location: quizzes.php');
        }
        //if saved
        else{
            echo "<script>alert('Quiz saved. Edit anytime!'); window.location.href = 'quizzes.php';</script>";            
        }
    }
      ...
      <!-- display question-->
        ...
        <div class="mt-2 text-center">
            <div class="d-grid gap-2 d-md-block">
                <a href="addquestion.php?id=<?php echo $quiz['quizID'];?>"><button class="btn btn-primary" type="button">Create Question</button></a>
                <a href="importquiz.php?id=<?php echo $quiz['quizID'];?>"><button class="btn btn-success" type="button">Import Question</button></a>
            </div>
        </div>
        <br>
        ...
        <form action="quiz.php" method="POST">
            <div class="mt-2 text-center">
                <button type="submit" class="btn btn-secondary" name="return">Return Home</button>
            </div>
        </form>
    </div>
 </body>
